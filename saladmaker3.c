...
int main(int argc , char ** argv){
	...
	if (shmid==-1){
		printf("Shared memory id not provided to saladmaker 3\n");
		exit(1);
	}
	if (smTime<0){
		printf("Invalid saladmaker time provided to saladmaker 3\n");
		exit(1);
	}
	...
	if(shmPtr==(tray*)-1){
		perror("shared memory attachment error in saladmaker 3\n");
		exit(2);
	}
	//open semaphores created by chef
	sem_t *chefSemaphorePtr = sem_open(chefSemaphore, 0);
    if(chefSemaphorePtr == SEM_FAILED){
        perror("chef semaphore not opened in saladmaker 3\n");
        exit(3);
    }
    sem_t *sm3Ptr = sem_open(saladmaker3_Semaphore, 0);
    if(sm3Ptr == SEM_FAILED){
        perror("saladmaker3 semaphore not opened in saladmaker 3\n");
        exit(3);
    }
	sem_t *mutexPtr = sem_open(mutex, 1);
    if(mutexPtr == SEM_FAILED){
		perror("mutex semaphore not opened in saladmaker 3\n");
        exit(5);
    }
	sem_t *waitsm3Ptr = sem_open(wait3_Semaphore, 0);
    if(waitsm3Ptr == SEM_FAILED){
		perror("waitsm3 semaphore not opened in saladmaker 3\n");
        exit(5);
    }
	...
	if (fPtr == NULL)
    {
        ...
        printf("\nUnable to open logfile in saladmaker3.\n");
        exit(EXIT_FAILURE);
    }
	fclose(fPtr);
	...
	while(true){
		...
		if (shmPtr->task_done){
			//store weights
			shmPtr->weightList[2].tomatoW=tomatoWeight;
			shmPtr->weightList[2].onionW=onionWeight;
			shmPtr->weightList[2].greenPepperW=greenpepperWeight;
			shmPtr->weightList[2].totalW=totalWeight;
			//store waiting and working times
			shmPtr->waitingTime[2]=totalWait;
			shmPtr->workingTime[2]=totalWork;
			//release mutex lock
			sem_post(mutexPtr);
		...
		fprintf(fPtr,"Saladmaker3 started making salad at time: %f\n",(startWork.tv_sec-shmPtr->chefInitTime.tv_sec)+(1e-6)*(startWork.tv_usec-shmPtr->chefInitTime.tv_usec));
		fclose(fPtr);
		printf("\nVegetables received by saladmaker 3, saladmaking in process\n");
		...
		tomatoWeight+=100*RandomFloat(0.8,1.2);
...
{
	float x_ms , y_ms , diff;
	
	x_ms = (float)x.tv_sec*1000000 + (float)x.tv_usec;
	y_ms = (float)y.tv_sec*1000000 + (float)y.tv_usec;
	
	diff = (float)y_ms - (float)x_ms;
